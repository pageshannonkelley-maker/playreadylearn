...
Please suggest 2-3 simple, joyful activities they can do RIGHT NOW. Keep it warm, encouraging, and easy to follow.`;
    setShowChat(true);
    setLoading(true);
      ...
      <div style={{ maxWidth: "560px", margin: "24px auto", padding: "0 16px" }}>
        ...
        {showChat && (
          <div style={{ background: COLORS.white, border: `2px solid ${COLORS.border}`, borderRadius: "16px", padding: "24px", marginTop: "16px" }}>
            ...
            {messages.filter(m => m.role === "assistant").map((msg, i) => (
              <div key={i} style={{ fontSize: "15px", color: COLORS.text, lineHeight: "1.7", marginBottom: "16px", whiteSpace: "pre-wrap" }}>
                {msg.content}
              </div>
            ))}
            ...
            {!loading && messages.length > 0 && (
              <div style={{ marginTop: "16px", borderTop: `1px solid ${COLORS.border}`, paddingTop: "16px" }}>
                ...
                <div style={{ display: "flex", gap: "8px" }}>
                  ...
                  />
                  <button onClick={sendFollowUp} style={{ padding: "10px 16px", borderRadius: "8px", border: "none", background: COLORS.accent, color: "#fff", fontWeight: "bold", cursor: "pointer", fontFamily: "Georgia, serif" }}>
                    Send
                  </button>
                </div>
              </div>
        ...
        {showChat && !loading && (
          <div style={{ textAlign: "center", marginTop: "16px" }}>
            <button onClick={() => { setShowChat(false); setMessages([]); setAge(""); setItem(""); setEnergy(""); }}
